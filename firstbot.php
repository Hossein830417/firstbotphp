...
function buttonStart($chat_id){
	$button=array();
	$button[]=array(array('text'=>'🌴 حذف ربات 🍀'),array('text'=>'🍂 ساخت ربات 🍁'));
	$_button=array();
	...
	if($chat_id==ADMIN || in_array($chat_id,$admins)){
		$button[]=array(array('text'=>'❤️ مدیریت ❤️'));
	}
...
if(in_array($step[$chat_id],$robot_list) && isset($text)){
	...
	if($is_bot_robot==true){
		...
		file_get_contents('https://api.telegram.org/bot'.$text.'/setWebHook?url='.$UpAd.'/data/robots/'.$username_robot.'/index.php');
...
#delete robot
if($text=='🌴 حذف ربات 🍀' && $step[$chat_id]=='home'){
	if($my_robots!=null){
		$button=array();
		$number=-1;
...
#managemant
if($text=='❤️ مدیریت ❤️' && $step[$chat_id]=='home' && ($chat_id==$admin || in_array($chat_id,$admins))){
...
if($step[$chat_id]=='unwarning_user' && isset($text) && $text!=$s){
	...
	if(array_key_exists($text,$warning_list)){
		...
		backManagemant($chat_id,"⭕️کاربر [$first_name_user](tg://usid=$text) هیچ اخطاری ندارد ! !",null);
	}
}
#block robot
$s='بلاک کردن ربات';
